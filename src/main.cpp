...
public:
  ...
  void render(std::chrono::milliseconds const& delay) {
  ...
  void update(std::chrono::milliseconds const& delay) {
...
auto compute_pixel_cost(size_t pixel_count) {
  ...
  return std::chrono::milliseconds(frame / pixel_count);
}
...
int main() {
    ...
    auto render_speed = compute_pixel_cost<60>(scene->size());
