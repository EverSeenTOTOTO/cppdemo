...
struct slice_helper<L<Args...>, length> {
  ...
  using type = typename concat<
      typename std::conditional<std::is_same<typename List::first, never>::value,  // type_list<>
...
// 切片，从start开始，取length个，方法是先取[start, ...（到list尾部）]再利用slice_helper取[start,
// ...（length个）]
...
struct index_of<L<Args...>, T> {
  typedef L<Args...>   List;
  static constexpr int value = std::is_same<T, typename List::first>::value ? 0
                               : index_of<typename List::rest, T>::value == -1
                                   ? -1
                                   : 1 + index_of<typename List::rest, T>::value;
};
...
struct insert_at<L<Args...>, index, Ts...> {
  typedef L<Args...> List;
  using type =
      typename concat<typename concat<typename slice<List, 0, index>::type, L<Ts...>>::type,
                      typename slice<List, index, List::length - index>::type>::type;
};
...
struct remove_type<L<Args...>, T> {
  ...
  using type = typename std::conditional<
      ...
      typename remove_type<typename List::rest, T>::type,
      typename insert_at<typename remove_type<typename List::rest, T>::type, 0,
                         typename List::first>::type>::type;
};
...
struct remove_duplicate<L<Args...>> {
  ...
  using type = typename insert_at<
      typename remove_duplicate<typename remove_type<typename List::rest, First>::type>::type, 0,
      First>::type;
};
...
template <template <typename...> class L, template <typename> typename F, typename... Args,
          typename _>
struct map<L<Args...>, F<_>> {
  using List = L<Args...>;
  using type = std::conditional<List::length == 0, List,
                                concat<L<typename F<typename List::first>::type>,
